...
int main() {
  ...
  while(true) {
    ...
    uint32_t bmillis = millis();
    uint32_t dmillis = bmillis - amillis;
    amillis = bmillis;

    char text[64];

    sprintf(text, "Hello World! %4lu ms/frame", dmillis);

    pico_display.set_pen(255, 255, 255);
    pico_display.text(text, text_location, 320);
